...
function App() {
  ...
  // // Gán giá trị khi students thay đổi
  // useEffect(() => {
  //   // khởi tạo một list student để hiển thị khi tải trang
  ...
  const handleSearch = (value) => {
    ...
    list = fn_search(list, value);
    list = fn_Sort(list, sort.type, sort.order);
    // Cập nhật lại state list student với kết quả tìm kiếm
    setStudents(list);
  };
  ...
  const fn_Sort = (list, type, order) => {
    ...
    // console.log("fn_Sort: ", type, order);
    return list;
  }

  const handleSort = (event) => {
    // console.log("App.jsx handleSort: ", event);
    let { type, order } = event; 
    let list = getLocalStorage();
 
    list = fn_search(list, inputSearch);
    // console.log("App.jsx search list: ", list);
    list = fn_Sort(list, type, order);
    // console.log("App.jsx sort list: ", list);
    setSort({ type, order });
    setStudents(list);
  };
        ...
        <div className="col-lg-7 grid-margin stretch-card">
          <div className="card">
            <StudentHeaderComponent
              ...
              onIsToogle={(evt) => handlerIsToogle(evt)}
              onSort={(evt) => handleSort(evt)}
            />
